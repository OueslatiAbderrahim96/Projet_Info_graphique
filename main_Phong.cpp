...
class Vector {
 ...
 Vector reflexion(const Vector& V,const Vector& N){
	Vector res = V - 2.* dot(V,N)*N;
	return res;
 }
...
Vector random_cos(const Vector& N){
	...
	double factor = sqrt(1-r2);
	Vector direction_aleatoir_repere_local(cos(2 * MATH_PI * r1) * factor, sin(2 * MATH_PI * r1) * factor,sqrt(r2));
...
Vector random_Phong(const Vector& Refl, double expo){
	double r1 = distrib(engine[omp_get_thread_num()]);
	double r2 = distrib(engine[omp_get_thread_num()]);
	double factor = sqrt(1 - std::pow(r2, 2./(expo+1)));
	Vector direction_aleatoir_repere_local(cos(2 * MATH_PI * r1) * factor, sin(2 * MATH_PI * r1) * factor, std::pow(r2, 1/(1+expo)));
	Vector aleatoir(distrib(engine[omp_get_thread_num()])-0.5,distrib(engine[omp_get_thread_num()])-0.5,distrib(engine[omp_get_thread_num()])-0.5);
	Vector tangent1 = cross(Refl,aleatoir);
	tangent1.normalize();
	Vector tangent2 = cross(tangent1, Refl);                            
	return direction_aleatoir_repere_local[2] * Refl + direction_aleatoir_repere_local[0] * tangent1 + direction_aleatoir_repere_local[1] * tangent2;
}
...
public :
const Vector A, B, C;
Triangle(const Vector& A, const Vector& B, const Vector& C, const Vector& albedo, double coeff =0., double exponent =1000., bool mirro = false, bool transp = false) :A(A), B(B), C(C){
	this->albedo = albedo;
	this->mirro = mirro; 
	this->transp = transp;
	this->coeff = coeff;
	this->exponentPhong = exponent;
...
public:
    Sphere(const Vector& O, double R, const Vector& albedo, double coeff =0., double exponent =1000., bool mirro = false, bool transp = false): O(O),R(R){
	this->albedo = albedo;
	this->mirro = mirro; 
	this->transp = transp;
	this->coeff = coeff;
...
public:
	Geometry() {};
	Geometry(const char* obj, double scaling, const Vector& offset, const Vector& albedo, double coeff, double exponent, bool mirro=false,bool transp=false) {
                this->albedo = albedo;
                this->mirro = mirro;
                this->transp = transp;
		this->coeff = coeff;
       ...
       bool intersect(const Ray& d, Vector& P, Vector& N, double& t) const {
		...
		while(!listBVH.empty()){
			...
			if(!currentNode->fg){                                
				for(int i=currentNode->i0; i<currentNode->i1 && !has_inter;i++){                                       
...
public :
   ...
   double BRDFPhong(const Vector& wi, const Vector& wo, const Vector& N, double exponent){
	
	Vector reflected = reflexion(wo,N);
	double res = std::pow(dot(reflected,wi),exponent) * (exponent + 2)/(2.*MATH_PI);
	return res;
  }
	...
	Vector intensite_pixel=Vector(0.,0.,0.);
        if(has_intersection){//cout<<objects.size()<<" ;ok  "<<indice_sphere<<endl;
		  ...
		  if(objects[indice_sphere]->transp){
			    ...
			    Ray lightRay(P+0.001*N, wi);     
			    Vector Pprime, Nprime;
			    int indiceprime;
			    double tprime;
                            bool has_inter = this->intersect(lightRay,Pprime, Nprime, indiceprime,tprime);
                             
                            if(has_inter && tprime*tprime < d_lightlocal*0.99){
				 intensite_pixel = Vector(0.,0.,0.);
			    }else{//if(indice_sphere==7) cout<<"ok1 : "<<indice_sphere<<endl;
                             intensite_pixel = (intensiteL / (4*MATH_PI*d_lightlocal)*std::max(0.,dot(N,wi))*dot(dir_aleatoire,-1*wi) /dot(axePO, dir_aleatoire))* MATH_PI*((1.-objects[indice_sphere]->coeff)*objects[indice_sphere]->albedo / MATH_PI + BRDFPhong(wi,r.u,N,objects[indice_sphere]->exponentPhong)*objects[indice_sphere]->coeff * objects[indice_sphere]->albedo) ;                        
                             }
                             //eclairage indirect      
                        Vector direction_aleatoir;
			double proba = 1- objects[indice_sphere]->coeff ;
			bool sampleDiffusion;
			Vector refl = reflexion(r.u,N);
			if(distrib(engine[omp_get_thread_num()])< proba){
						direction_aleatoir = random_cos(N);
						     sampleDiffusion = true;
						    
			}else{
				direction_aleatoir = random_Phong(refl, objects[indice_sphere]->exponentPhong);
						     sampleDiffusion = false;
			if(dot(direction_aleatoir,N)<0) return Vector(0., 0., 0.);
			if(dot(direction_aleatoir,refl)<0) return Vector(0., 0., 0.);
				}
				        Ray rray_aleatoir(P+0.001*N, direction_aleatoir);

					double PhongP = (objects[indice_sphere]->exponentPhong +1)/(2.*MATH_PI) * std::pow(dot(refl, direction_aleatoir),objects[indice_sphere]->exponentPhong);
					double proba2 = proba * dot(N, direction_aleatoir)/(MATH_PI)+(1-proba)*PhongP;

			if(sampleDiffusion)
				intensite_pixel += getColor(rray_aleatoir, numrebond-1) * objects[indice_sphere]->albedo * dot(N, direction_aleatoir/MATH_PI)/proba2;
			else
				intensite_pixel += getColor(rray_aleatoir, numrebond-1) * dot(N, direction_aleatoir) * BRDFPhong(direction_aleatoir,r.u,N,objects[indice_sphere]->exponentPhong) * objects[indice_sphere]->coeff * objects[indice_sphere]->albedo/proba2;
		       }
        	}
	}
...
int main()
{
    ...
    int H = 512;
    Scene s;
    Sphere s_lumiere(Vector(10, 30, 30),40.,Vector(1.,1.,1.));
    ...
    Geometry g1("BeautifulGirl.obj",25.,Vector(0.,-18.,-30.),Vector(1.,1.,1.),0.3, 1000.);
    Sphere ssol(Vector(0., -1000+40, 0.), 1000-60, Vector(1.,1.,1.),0.0, 0.);
    Sphere splafond(Vector(0., 1000-40, 0.), 1000-60, Vector(1.,1.,1.),0.0, 0.);
    Sphere smurgauche(Vector(-1000+40, 0., 0.), 1000-60, Vector(1.,0.,0.),0.0, 0.);
    Sphere smurdroit(Vector(1000-40, 0., 0.), 1000-60, Vector(0.,0.,1.),0.0, 0.);
    Sphere smurfond(Vector(0., 0., -1000), 1000-60, Vector(0.,1.,0.),0.0, 0.);
